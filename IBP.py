    ...
    def run_IBP(self, model, input, store_linear_layers=True):
        ...
        for layer_idx, layer in enumerate(model):
            if isinstance(layer, nn.Linear):
                ...
                layer_information.loc[len(layer_information), :] = {'Layer_idx': layer_idx, 'Layer_type': "nn.Linear",'Layer_input_bounds': pre_activation_bounds.detach().cpu(), 'Layer_output_bounds': pre_layer_input.detach().cpu()}
            ...
            if isinstance(layer, nn.ReLU):
                ...
                # The layer information is saved here in a dataframe
                layer_information.loc[len(layer_information), :] = {'Layer_idx': layer_idx, 'Layer_type': "nn.ReLU",'Layer_input_bounds': pre_activation_bounds.detach().cpu(), 'Layer_output_bounds': pre_layer_input.detach().cpu()}
