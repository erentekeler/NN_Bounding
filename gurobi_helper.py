...
def constrain_ReLU(m, layer, layer_idx, layer_input, model_type):
    ...
    ReLU_input_var = m.addMVar(n_neurons, lb=layer["Layer_input"][:,0], ub=layer["Layer_input"][:,1], name=f'z_{layer_idx}') 
    m.addConstr(ReLU_input_var == layer_input) # The previous layer output is the input to this layer

    # Creating the post activation variables for the ReLU layer
    p_activation_var = m.addMVar(n_neurons, lb=-gp.GRB.INFINITY, name=f'z_hat_{layer_idx}')
    ...
    if model_type == 'triangular':
        ...
        if b_positive_mask.any():
            m.addConstr(p_activation_var[b_positive_mask] == ReLU_input_var[b_positive_mask])

        # Both negative
        if b_negative_mask.any():
            m.addConstr(p_activation_var[b_negative_mask] == 0)

        # Both zero
        if b_zero_mask.any():
            m.addConstr(p_activation_var[b_zero_mask] == 0)

        # lb negative, ub positive
        if unstable_mask.any():
            m.addConstr(p_activation_var[unstable_mask] >= lower_slope[unstable_mask]*ReLU_input_var[unstable_mask] + lower_bias[unstable_mask]) # lower bound y=0
            m.addConstr(p_activation_var[unstable_mask] >= ReLU_input_var[unstable_mask]) # lower bound y=x
            m.addConstr(p_activation_var[unstable_mask] <= upper_slope[unstable_mask]*ReLU_input_var[unstable_mask] + upper_bias[unstable_mask]) # upper bound, IBP computed
